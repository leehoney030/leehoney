...
esnake_pos = [300, 200]
esnake_body = [[300, 200], [300-10, 200], [300-(2*10), 200], [300-(3*10),200]]

food_pos = [random.randrange(1, (frame[0]//10)) * 10,
            random.randrange(1, (frame[1]//10)) * 10]
food_spawn = True

pfood_pos = [random.randrange(1, (frame[0]//10)) * 10,
            random.randrange(1, (frame[1]//10)) * 10]
pfood_spawn = True
...
def show_coin(window, size, choice, color, font, fontsize):
    # Coin를 띄우기 위한 설정입니다.
    # Settings for showing coin on screen
    coin_font = pygame.font.SysFont(font, fontsize)
    coin_surface = coin_font.render('coin : ' + str(coin), True, color)
    coin_rect = coin_surface.get_rect()

    if choice == 1:
        coin_rect.midtop = (size[0]/10, 40)
    else:
        coin_rect.midtop = (size[0]/2, size[1]/1.25)

    # 설정한 글자를 window에 복사합니다.
    # Copy the string to windows
    window.blit(coin_surface, coin_rect)
...
def get_keyboard(key, cur_dir):
    ...
    if i_direction != 'DOWN' and (key == pygame.K_UP or key == ord('w')):
        return 'UP'
    if i_direction != 'UP' and (key == pygame.K_DOWN or key == ord('s')):
        return 'DOWN'
    if i_direction != 'RIGHT' and (key == pygame.K_LEFT or key == ord('a')):
        return 'LEFT'
    if i_direction != 'LEFT' and (key == pygame.K_RIGHT or key == ord('d')):
        return 'RIGHT'
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                i_direction = get_keyboard(event.key, i_direction)


    #1~4까지 랜덤한 숫자를 rand_direction에 부여.
    rand_direction = random.randrange(1,5)
    ...
    esnake_body.insert(0, list(esnake_pos))
    esnake_body.pop()
    ...
    if snake_pos[0] == pfood_pos[0] and snake_pos[1] == pfood_pos[1]:
        score -= 1
        snake_body.pop()
    ...
    pygame.draw.rect(main_window, orange,
                     pygame.Rect(pfood_pos[0], pfood_pos[1], 10, 10))
    # Game Over 상태를 확인합니다.
    ...
    # 뱀이 적 뱀에게 닿았는지 확입합니다.
    for i_pos in snake_body:
        for e_pos in esnake_body:
            if e_pos[0] == i_pos[0] and e_pos[1] == i_pos[1]:
                game_over(main_window, frame)

    # 적 뱀이 벽에 닿으면 튕기게 합니다.
    if esnake_pos[0] < 0:
        esnake_pos[0] += 10
    if esnake_pos[0] > frame[0] - 10:
        esnake_pos[0] -= 10
    if esnake_pos[1] < 0:
        esnake_pos[1] += 10
    if esnake_pos[1] > frame[1] - 10:
        e_direction -= 10
    ...
    show_coin(main_window, frame, 1, white, 'consolas', 20)
